package io.github.xenogew.propedit.eclipse.plugin.jdt.proposal;
...
public class PropertiesCompletionProposalComputer implements IJavaCompletionProposalComputer {
  ...
  public List<ICompletionProposal> computeCompletionProposals(
    ...
    List<String> keyList =
        new ArrayList<>(ProjectProperties.getInstance().getProperty(project).stringPropertyNames());

    String source = context.getDocument().get();
    int offset = context.getInvocationOffset();
    int idx = source.charAt(offset) == '\"' ? source.lastIndexOf("\"", offset - 1) //$NON-NLS-1$
        : source.lastIndexOf("\"", offset); //$NON-NLS-1$
    ...
    for (String key : keyList) {
      if (key.startsWith(match)) {
        list.add(
            new CompletionProposal(key, offset - match.length(), match.length(), key.length()));
      }
    }
