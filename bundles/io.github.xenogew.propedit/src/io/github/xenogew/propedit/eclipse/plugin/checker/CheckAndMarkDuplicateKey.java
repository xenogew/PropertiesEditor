...
import io.github.xenogew.propedit.eclipse.plugin.preference.PropEditorXDuplicationCheckerPreference;
...
public class CheckAndMarkDuplicateKey {
  ...
  public void checkAndMarkDuplicateKeyInResource(IResource resource) throws CoreException {
    ...
    try {
      ...
      PropEditorXPlugin.getDefault().getLog().log(coreStatus);
      throw e;
    } catch (Exception e) {
      IStatus status =
  ...
  public void checkAndMarkDuplicateKeyInString(String text, IResource resource)
    ...
    // delete marker
    IMarker[] markers = resource.findMarkers("io.github.xenogew.propedit.duplicationmarker", //$NON-NLS-1$
    ...
    if (!PropEditorXPlugin.getDefault().getPreferenceStore()
        .getBoolean(PropEditorXDuplicationCheckerPreference.P_CHECK_KEY)) {
      return;
    }
    ...
    for (var entry : keyMap.entrySet()) {
      ...
      if (lines.size() > 1) {
        for (int lineNum : lines) {
          IMarker marker = resource.createMarker("io.github.xenogew.propedit.duplicationmarker"); //$NON-NLS-1$
