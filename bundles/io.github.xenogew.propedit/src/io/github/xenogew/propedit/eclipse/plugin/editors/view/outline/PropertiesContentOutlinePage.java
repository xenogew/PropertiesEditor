...
public class PropertiesContentOutlinePage extends ContentOutlinePage {
  protected static String ALPHA_SORT_BTN_CHECK_KEY =
      "io.github.xenogew.propedit.eclipse.plugin.editors.alphaSortCheckKey"; //$NON-NLS-1$
  ...
  public void createControl(Composite parent) {
    ...
    PropEditorXPlugin.getDefault().getPreferenceStore().setDefault(ALPHA_SORT_BTN_CHECK_KEY, false);
    ...
    Action action = new Action(label, Action.AS_CHECK_BOX) {
      public void run() {
        PropEditorXPlugin.getDefault().getPreferenceStore().setValue(ALPHA_SORT_BTN_CHECK_KEY,
            isChecked());
        if (isChecked()) {
    ...
    if (sortImageDescriptor == null) {
      URL url = PropEditorXPlugin.getDefault().getBundle().getEntry("/icons/alphab_sort_co.png"); //$NON-NLS-1$
    ...
    action.setChecked(
        PropEditorXPlugin.getDefault().getPreferenceStore().getBoolean(ALPHA_SORT_BTN_CHECK_KEY));
    action.run();
  }
