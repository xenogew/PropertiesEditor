  ...
  public static String getCommentChar(IProject project, String preference) {
    ...
    try {
      org = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, PropertiesProperty.P_ORIGINAL_SETTINGS));
      commentChar = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, PropertiesProperty.P_COMMENT_CHARACTER));
  ...
  public static boolean getNotAllConvert(IProject project, boolean preference) {
    ...
    try {
      ...
      notAllConvert = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, PropertiesProperty.P_NOT_ALL_CONVERT));
    } catch (CoreException e) {
      IStatus status =
          new Status(IStatus.ERROR, PropEditorXPlugin.PLUGIN_ID, IStatus.OK, e.getMessage(), e);
      ILog log = PropEditorXPlugin.getDefault().getLog();
  ...
  public static boolean getNotConvertComment(IProject project, boolean preference) {
    ...
    try {
      ...
      notConvertComment = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, PropertiesProperty.P_NOT_CONVERT_COMMENT));
    } catch (CoreException e) {
      IStatus status = new Status(IStatus.ERROR, PropEditorXPlugin.PLUGIN_ID, 0, e.getMessage(), e);
      ILog log = PropEditorXPlugin.getDefault().getLog();
      log.log(status);
      ErrorDialog.openError(null,
          Messages.getString("eclipse.propertieseditor.property.error_title"), //$NON-NLS-1$
  ...
  public static String getCharCase(IProject project, String preference) {
    ...
    try {
      ...
      charcase = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, PropertiesProperty.P_CONVERT_CHAR_CASE));
    } catch (CoreException e) {
      IStatus status =
          new Status(IStatus.ERROR, PropEditorXPlugin.PLUGIN_ID, IStatus.OK, e.getMessage(), e);
      ILog log = PropEditorXPlugin.getDefault().getLog();
