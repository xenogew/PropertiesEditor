package io.github.xenogew.propedit.eclipse.plugin.property;
...
public class PropertiesProperty extends PropertyPage {
  ...
  protected Control createContents(Composite parent) {
    ...
    orgCheckBox
        .setText(Messages.getString("eclipse.propertieseditor.property.project_org_settings")); //$NON-NLS-1$
    ...
    notAllConvertCheckBox
        .setText(Messages.getString("eclipse.propertieseditor.preference.convert")); //$NON-NLS-1$
    ...
    notConvertCommentCheckBox
        .setText(Messages.getString("eclipse.propertieseditor.preference.convert.comment")); //$NON-NLS-1$
  ...
  private void initialize() {
    ...
    try {
      ...
      org = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_ORIGINAL_SETTINGS));
      commentChar = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_COMMENT_CHARACTER));
      charcase = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_CONVERT_CHAR_CASE));
      notAllConvert = project
          .getPersistentProperty(new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_NOT_ALL_CONVERT));
      notConvertComment = project.getPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_NOT_CONVERT_COMMENT));
  ...
  protected void performApply() {
    try {
      ...
      project.setPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_ORIGINAL_SETTINGS),
          Boolean.toString(orgCheckBox.getSelection()));
      project.setPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_COMMENT_CHARACTER),
          commentText.getText());
      project.setPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_CONVERT_CHAR_CASE),
          convertCharCaseCombo.getText());
      project.setPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_NOT_ALL_CONVERT),
          Boolean.toString(notAllConvertCheckBox.getSelection()));
      project.setPersistentProperty(
          new QualifiedName(PropEditorXPlugin.PLUGIN_ID, P_NOT_CONVERT_COMMENT),
          Boolean.toString(notConvertCommentCheckBox.getSelection()));
    } catch (CoreException e) {
      IStatus status =
          new Status(IStatus.ERROR, PropEditorXPlugin.PLUGIN_ID, IStatus.OK, e.getMessage(), e);
      ILog log = PropEditorXPlugin.getDefault().getLog();
      ...
      ErrorDialog.openError(getShell(),
          Messages.getString("eclipse.propertieseditor.property.error_title"), //$NON-NLS-1$
          Messages.getString("eclipse.propertieseditor.property.save.settings.error"), status); //$NON-NLS-1$
    }
  }
  ...
  protected void performDefaults() {
    commentText.setText("#"); //$NON-NLS-1$
    convertCharCaseCombo
        .setText(Messages.getString("eclipse.propertieseditor.preference.convert.char.lowercase")); //$NON-NLS-1$
    notAllConvertCheckBox.setSelection(false);
    notConvertCommentCheckBox.setEnabled(false);
    notConvertCommentCheckBox.setSelection(false);
    orgCheckBox.setSelection(false);
